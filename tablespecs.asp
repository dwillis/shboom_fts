...
HasFTI = Request("HasFTI")

HasFTI = CBool(HasFTI)

'Response.Write("Has FTI is: " & HasFTI & "<br>")
...
On Error Resume Next 
 ...
 ' If the file does not exist we will get error no 3709
  If err.number = 3709 Then
    ...
    objRS.Fields.Append "ShowFTS", adBoolean   
...
If HasFTI = True Then
	...
	sql2 = "exec sp_help_fulltext_columns " & thetable


	'set rs2 = Conn.Execute("sp_help_fulltext_columns " & tablename)
	RS2.Open sql2, Conn, adOpenStatic, adLockReadOnly, adCmdText
End If
...
'FTRC = rs2.RecordCount

'FTRC = rs2("fulltext_colid")
'Response.Write("Num of FT Columns: " & FTRC & "<br>")
...
Do While NOT rs.EOF
	...
	If HasFTI = FALSE Then

		objRS("ShowFTS") = FALSE
	Else	
		
		If objRS("ID") = rs2("FullText_ColID") Then
			objRS("ShowFTS") = TRUE
			rs2.MoveNext
		Else
			objRS("ShowFTS") = FALSE
...
<INPUT TYPE="HIDDEN" NAME="HasFTI" VALUE="<% = HasFTI %>">
    ...
    <table border="3" cellpadding="0" cellspacing="2" align="center">
      <tr>
        ...
        <td align="center" colspan="2" bgcolor="#F7ECCA">Show in <br>FTS Pulldown</td>
...
%>
	<INPUT TYPE="HIDDEN" NAME="ID" VALUE="<% = objRS("ID") %>">
      ...
      <tr bgcolor="<% = thecolor %>">
        ...
        <td><input type="text" name="frmTheAlias" size="15" value="<% = objRS("TheAlias") %>"></td>
...
If objRS("ShowFTS") = TRUE Then %>
        <td bgcolor="<% = thecolor %>">Y<input type="radio" name="<% = "frmShowFTS(" & J & ")" %>"  value="TRUE" checked></td><td bgcolor="#FDC0AC">N<input type="radio" name="<% = "frmShowFTS(" & J & ")" %>" value="FALSE"></td>
<% Else %>
        <td bgcolor="<% = thecolor %>">Y<input type="radio" name="<% = "frmShowFTS(" & J & ")" %>"  value="TRUE"></td><td bgcolor="#FDC0AC">N<input type="radio" name="<% = "frmShowFTS(" & J & ")" %>" value="FALSE" checked></td>

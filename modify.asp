...
If WhereFrom = "" Then
	...
	pkgInclude = rs("pkgInclude")
Else


	TheDB = Request("TheDB")
	TheTable = Request("TheTable")
	pkgInclude = Request("frmpkgInclude")
...
rs("DoFTI")=Trim(Request("frmDoFTI"))
rs("pkgInclude")=Trim(Request("frmpkgInclude"))
rs.Update
...
<% If rs("HasFTI") = True Then %>

      <tr>
        <td align="right"><b>Enable Full-text searching on this table
</td></b>
        <td><b><select size="1" name="frmDoFTI">
<% If rs("DoFTI") = "Yes" Then %>
            <option value="NO">No</option>
            <option selected value="YES">Yes</option>
...
<% End If %>
      ...
      <tr>
        <td align="right"><b><i>Virtual</i> path and name of include file for packages.</td></b>
        <td><b><input type="text" name="frmpkgInclude" size="75" value="<% = pkgInclude %>"></td></b>
      </tr>
      <tr>
